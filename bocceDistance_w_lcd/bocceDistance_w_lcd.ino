...
const int scePin = 3;   // SCE - Chip select, pin 3 on LCD.
...
//  digitalWrite(TRANSISTOR, HIGH);
  ...
  setStr("Bocce Distance Measurer", 0, 16, BLACK);
  updateDisplay();
...
if (lastValue != NULL)
{
//  setStr(lastValue, 0, 8, BLACK);
//  setStr(" cm", 5, 8, BLACK);
} 
  setStr("Idle", 0, 0, BLACK);
  updateDisplay();
...
{ 
  ...
  Serial.println("Ping: ");
    int pingResponse = sonar.ping_cm(); // Send ping, get distance in cm and print result (0 = outside set distance range)
    char pingResponseChar[3];
    String str;
    str = String(pingResponse);
    str.toCharArray(pingResponseChar,3);
    str.toCharArray(lastValue,3);
...
//  clearDisplay(WHITE); // Begin by clearing the display
  setStr(pingResponseChar, 0, 8, BLACK);
  setStr(" cm", 5, 8, BLACK);
  updateDisplay();
